...
function TodoList() {
  ...
  const addTodo = () => {
    // todos기본으로 아무런 리스트가 없는 빈 배열이존재
    // setTodos로 기본으로 아무런 리스트가 없는 목록에 새로운 목록 하나씩 추가하는 것을 진행
    //      [...todos] = 기존에 있던 리스트를 복사해서
    // value 값이 input으로 들어온 value 값을 text 라는 키에 저장
    // completed = 무언가를 끝내다 라는 영어
    // 할 일 추가 = 끝내야 하는 일
    // completed: false 새로 추가된 일은 아지 완료하지 않을 일이기 때문에 false
    // completed: false 내 할 일을 완료하지 않았음의 표시
  ...
  const toggleTodo = (index) => {
    ...
    //  newTodos    [index]                .completed = !newTodos[index].completed
    // 새로운목록들[사용자가 선택한 목록번호].완료/미완료 = 완료 -> 미완료, 미완료 -> 완료로 상태 변경
  ...
  return (
    <div className="TodoList">
      ...
      우리가 li태그를 클릭할 때마다 할 일을 완료했는지, 미완료인지 눈으로 확인할 수 있게 표기
