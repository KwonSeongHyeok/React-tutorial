...
function UserList() {
  ...
  // 빈 배열인 상태 use State([]); = 목록이 비어있음

  // 초기 변수 user 가 아무런 유저 목록을 담고 있지 않기 때문에 users = []; 빈 배열로 설정
  ...
  */
  ...
  3번
   ...
   input의 경우 사용자가 어떤 값을 작성할지 컴퓨터가 모르기 때문에
          사용자가 어떤 값을 입력한다는 것을 이벤트로 받아들이고, 이벤트가 감지되면 value 값을 가져옴
  ...
  */
  ...
  const 삭제버튼 = (index) => {
                        //users에 있는 유저목록   user = 유저명, i = users에 있는 유저가 저장된 번호
