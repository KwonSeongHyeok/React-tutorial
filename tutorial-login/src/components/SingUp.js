...
const Signup = () => {
    ...
    const 아이디중복검사 = (eventId) => {
        ...
        .then(result => {
            ...
            // 결과전달을 0을 사용하지 않고 "중복아님"글자 전달 if(result === "중복아님")
            if(Number(result) === 0) {
    ...
    const 회원가입버튼 = () => {
        ...
        */
        ...
        fetch("/signUp", {
            method : "POST", // Spring Boot Controller에 @PostMapping("/singUp")에 전달하겠다는 표시
            ...
            // 데이터를 전달할 때 이미지파일, 동영상 파일인지, 다중 이미지 파일인지 글자만 있는지 정보 전달
            // application/json : application=코딩하는 폴더 자체를 의미 폴더 1개 = 어플리케이션 1개
            // json = 사용자나 개발자가 작성한 여러값을 key-value형태로 데이터 1개 이상 주고 받을 수 있는 형태
        ...
        .then(response => response.text())
        .then(result => {
            if(Number(result) > 0) { // 결과가 0보다 크다면 == 1이면 회원가입 완료 처리
                setResult('회원가입성공!~');
                // input값들 모두 초기화
                setId('');
                setPw('');
    ...
    return(
        <div className="signup-container">
            ...
            <label><input/></label> 작성해주면 for를 작성한 것과 동일하게 설정
