...
const Child = () => {
    ...
    const 사용자추가하기 = () => {
        ...
        // ...변수명 = 변수명에 담긴 모든 값을 복사해 놓겠다.
        const 새로운유저 = [...userList, 유저];
        // 기존에 있는 내용을 복제하고 복제한 리스트에 새로운유저를 추가해서 덮어쓰기

        setUserList(새로운유저); // 새로운유저가 추가된 리스트로 변경해!
    }

    return(
        <div>
            <laber htmlFor="inputId">이름</laber>
            <input type="text"
            id="inputId"
            onChange={(e) => {setInputName(e.target.value)}}
            value={inputName} />
            <br></br>
            <laber htmlFor="inputAge">나이</laber>
            <input type="number"
            id="inputAge"
            onChange={(e) => {setInputAge(e.target.value)}}
            value={inputAge} />

        {/* onChange={(e) => {setInputNeme(e.target.value)}}
