...
const 예제3번 = (props) => {
    // props로 전달받을 값 중 init값을 conunt 초기 값으로 설정하기

    // setCount()를 이용해서 count값을 변경하는 코드를 작성
    // -> 예제 2번 객체에서 변경이 되는 부분만 다시 표현 진행

    // JavaScript안에 Number라는 객체를 이용해서 받아오는 값을 숫자로 한 번 더 처리
    // String처럼 문자열로 오는 값일 수 있으니 한 번 더 숫자화 시켜줌
    /*
    useState(숫자로변경(App.js에서 가져온 값. 초기값(=init)이라는 변수에 변수에 담긴 값))
